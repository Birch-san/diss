define(["lib/jquery-2.1.0.min",£"lib/FileSaver", "trace"],£function(jq, saver, tracer) {
  ...
  function makeEvaluators() {    
    var keyCountElem = makeKeyCount(d£ocument.documentElement);
...
checkNewFocus(document.activeElement,£window.location);
  ...
  function£evaluatorIncrementKeys(theKey) {
    ...
    var thisDelta = nowTime-(evaluato£r.timer+evaluator.totalTime);
...
//evaluator.keyCountElem.innerHTML =£keyCount;
  ...
  function£evaluatorWriteState(clickedElem) {
    ...
    //console.log(bb);
    //bb.append((new XMLSerializer).s£erializeToString(document));
    //var blob = bb.getBlob("applicat£ion/xhtml+xml;charset=" +£document.characterSet);
 ...
 return {makeEvaluators:makeEvaluators,
        ...
        evaluatorIncrementKeys:evalua£torIncrementKeys,
        evaluatorWriteState:evaluator£WriteState,
        checkNewFocus:checkNewFocus,
        checkFocusAfterWait:checkFocu£sAfterWait};
});
