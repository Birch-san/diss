...
% less deprecated subfigure
\usepackage{subcaption}

%highlighting
\usepackage{color}
% involved in the JavaScript syntax highlight
\usepackage{upquote}
...
%,bibencoding=utf8
\usepackage[firstinits=true, sorting=nyt,backend=bibtex]{biblatex}
...
\lstdefinestyle{whatever}{%
    ...
    prebreak=\raisebox{0ex}[0ex][0ex]{\ensuremath{\color{red}\rhookswarrow\space}},
...
\newcommand{\disstitle}{ChibiPoint: Accessible Pointing for Web Applications}
\newcommand{\myname}{Alexander Birch}
\title{\disstitle}
\author{Alexander Birch}
\date{Bachelor of Science in Computer Science with Honours (incl. ab initio Japanese Language \& with Industrial Placement)\\The University of Bath\\April 2014}
...
\declaration{\disstitle}{\myname}


\abstract
The web is designed for pointing devices like mice. Many users cannot use such input devices, due to disability or device restrictions. The existing accessible standard for mouseless pointing on the web is `tabbing navigation', which we find to be ineffective, unpredictable and inefficient. In order to solve this problem, we design, develop and evaluate `ChibiPoint': a pointing mechanism designed for modern websites. ChibiPoint excels in providing pointing in previously-inaccessible websites, contributing an improved accessible mechanism for pointing, and relieving web developers of the expensive task of developing for accessibility. We find that the web browser itself is a strong vector for inserting accessibility, and recommend that web technologies be used to fix the accessibility gap on the user's end, rather than relying on website developers to be accommodating.
\newpage
...
We confirm in our Literature Review (\Cref{chap:litreview}) the need for an alternative pointing mechanism, and explore the approaches that have been attempted already.
...
\textit{We present these hypotheses --- and their assessment criteria --- in further detail in \Cref{sec:detailedhyp}}.
...
% Unfortunately this mechanism is ineffective~\cite{hoffman2005emerging}, unpredictable~\cite{hoffman2005emerging} and inefficient~\cite{schrepp2006efficiency}.
...
\chapterref{chap:evaluation}{Evaluation} explains how we evaluated the system in two studies: a usability study, and a larger quantitative study.
\chapterref{chap:results}{Results \& Analysis} describes and analyses the results of our larger quantitative study. We determine how our web browser extension compares to tabbing navigation, and determine also the contribution of our novel algorithm for suggesting `clickables' (defined in \Cref{sec:clickablesExplained}).
\chapterref{chap:discussion}{Discussion} discusses the results and analysis of the quantitative study, and ultimately decides how much support exists for our hypotheses and assertions about our system.
...
\end{enumerate}
\newcounter{existingapproach}
\newcommand{\anexistingapproach}[1]{\stepcounter{existingapproach}\subsubsection{\arabic{existingapproach}.~#1}}
...
Controls can be focused and clicked using the keyboard via `tabbing navigation'. \Cref{fig:tabjourney} illustrates the process of moving keyboard focus one element at a time by repeatedly pressing Tab. In this way, a user can traverse to the element that they want to select, and send a `click' to it (by pressing Return).

\begin{figure}[ht]
\centering\includegraphics[width=\textwidth]{figures/TabbingJourney.png}
\caption{A tabbing `journey': focus travels from button to button, one keypress at a time}
...
\anexistingapproach{Using web technologies to augment accessibility}
  Whilst it is possible (to an extent) to create \glspl{keybinding} in web content (e.g. via access keys, \Cref{accesskeysection}, or through JavaScript event libraries like MouseTrap~\cite{Mousetrap}), for the most part web applications do not make use of this. Possible reasons for this are:
...
\anexistingapproach{Changing the web browser}
...
\reqstart
	...
	\reqitem{Fast navigation}{The goal is `efficient' navigation, not `fast' navigation. There can exist a correlation (less inputs will, from a GOMS perspective, necessarily reduce task time also, provided cognitive and perceptual factors are negligible). But efficiency is the more important battle, as the system is targeted at low-input-bandwidth users, such as those with RSI (who need to avoid repetitive motor activity).}
...
Our software solution will need to cope with the many and varied pointing scenarios presented in typical web applications like \Cref{fig:typicalWebAppLayout}. Some discrete cases of interest have been labelled; a discussion follows for each of these numbered cases, regarding what is significant about them, and how we could consider optimizing our software for such cases. We prescribe also the design criteria we will follow to avoid falling afoul of these problems.

\begin{figure}[H]
\centering\rotatebox{-90}{\includegraphics[height=\textwidth,width=\safefigheight,keepaspectratio]{figures/LowFidelityFacebookLabelled}}
...
\someprob{Infinite Scrolling}{Our system needs to provide absolute traversal, so that it does not get stuck in focus traps like infinite scrolling content.}
...
That is: the viewport of the browser is divided into a ($3\times3$) grid, and each cell assigned a keyboard shortcut. We illustrate this in \Cref{fig:grid9}; cell keyboard shortcuts are mapped to the layout of the numpad.
...
Grids are constructed within grids, and a set of `crosshairs' (like on the sight of a gun) can follow the center of the most specific grid, and provide mouse behaviour (sending `click' and `hover' events) at those coordinates. Such crosshairs can be seen aiming at the `About' button in \Cref{fig:crosshairs}.
...
We provide in \Cref{fig:clickables} a picture to illustrate what clickables could exist on a webpage. These are elements the user might want to interact with, which listen for click events.
\begin{figure}[ht]
...
We provide in \Cref{fig:suggestions} a picture to illustrate a `shortlist' of clickables being made for a user to choose from. No intelligence is used in picking this illustrative shortlist, but it demonstrates that some clickables have been filtered out, and a `top few' by some criteria are suggested to the user. These suggestions will be keyboard-activated, and thus need to display labels of the shortcut that invokes them, as well as indicating which element, upon invocation, will be clicked.
...
\FloatBarrier
Suggestions can complement the grid-drilling approach. They can be re-evaluated and constrained to the space of the latest grid, every time the user `drills down', allowing increasingly more informed suggestions to be made. \Cref{fig:filteredsuggestions} illustrates how a shortlist can be targeted to produce suggestions from within only a specified area.
...
We illustrate in \Cref{fig:flyouts} what a flyout might look like. Clearly if labels were clipped directly to these interface elements, the labels would occlude nearby clickables. Flyouts tag a clickable from afar to improve visibility.
\begin{figure}[ht]
...
\FloatBarrier
If many flyouts are densely-packed, there exists a risk that flyouts will cover each other up, or the elements for which they are responsible. Hence we will implement some avoidance strategy to make them fan out from their respective targets in separate directions (as in the idealistic illustration of \Cref{fig:flyouts}), allowing the clickables beneath to be seen. Additionally we shall endeavour to limit the number of flyouts.
...
A discussion on StackOverflow.com provided the answer we needed~\cite{eventlistenerprototype}: before the \gls{dom} is constructed, overwrite the definition of `\texttt{addEventListener}' used in the prototype for HTML elements. In this way, we can ensure that, any time an event listener is added to an element, it is tracked in a list on that element (which we can later query for). This code can be seen in \Cref{lst:handlerTypes}. Once an attribute exists denoting an element's having click events, it becomes trivial to (using JQuery) select all elements in the page which have this list attributed to them.
...
The code used for querying for clickables can be seen in \Cref{lst:detectclickables}.
...
Part of this algorithm can be seen implemented in code in \Cref{lst:codeshortlist}. The code is not production-quality, so we recommend relying on the written algorithm for understanding.

An example of our system generating a shortlist of clickables with this algorithm can be seen in \Cref{fig:origClickable}. This screenshot also showcases an alternative key-mapping for drilling (QWE | ASD | ZXC instead of numpad). It is worth declaring also that we revise the visibility of these flyouts before the final evaluation of the system (\Cref{sec:interfaceprobs}).
...
Should the user find that their intended target is not captured in this shortlist, they can further specify their space of interest, by drilling down into a quadrant. This search refinement is demonstrated in \Cref{fig:impFilteredClickable}.

% \begin{figure}[ht]
% \centering\includegraphics[width=\textwidth]{figures/ImpFilteredClickables.png}
...
We illustrate this problem of converging towards a grid edge in \Cref{fig:growingoff}.
...
Flyouts fan out in the direction that leads away from the center of the most specific grid. This allows them to choose unique directions without communicating with each other. This behaviour can be seen in \Cref{fig:impFilteredClickable}.
...
The primary system comparison we sought to pursue was `ChibiPoint' versus `tabbing navigation'. We aimed also to evaluate the contribution made by the `flyouts' feature of ChibiPoint: whether it helps or hinders. The exact qualities we intended to compare are detailed in \Cref{sec:systemcomparisonobjectives}.
...
The participant's actions were tracked using screen recording. Voice was recorded, and as such a full transcript of the session can be found in \Cref{lst:usabilityTrans}.
...
The mean average age for participants was 22.58 (SD=2.15). The range was 21--28 (\Cref{fig:partic_age}).
...
The study began with the first questionnaire's being allocated (\Cref{fig:prequestionnaire}). This asked participants to disclose their demographic (occupation, gender and age).

The participants began the testing activity by reading instructions on the system they had been allocated. Such instructions can be seen in \Cref{sec:instructions}. The researcher then guided them through a training scenario to confirm that they had understood the instructions. During this training period, the researcher would answer any questions and correct any mistakes that occurred. In the case of learning the system `ChibiPoint with crosshairs AND flyouts' before learning the system `ChibiPoint with crosshairs ONLY', participants were trained in the use of both features in turn.

After training, all test websites were opened, and the participant was directed to the attend to the first \gls{browsingcontext}, which was navigated to the website for task 1. The researcher would point out to them the page element that needed to be clicked in this task. The pointing objectives were also printed on a piece of paper that they could refer to (\Cref{fig:taskscript}).
...
With all systems tested, the participants were finally asked to fill in a post-experiment questionnaire (\Cref{fig:postquestionnaire}), describing their preferences and thoughts on the systems tested.
...
\begin{enumerate}
	...
	\TaskDef{[Within form] Spatially close, markup distant traversal}{navigating to a YouTube search page from the front-page search guarantees that your focus starts in the `search' field. From here, the first search result is right next to the search form, but it is a far longer journey in the page markup. Click that first search result.}{This test shows that visual proximity is, at best, only a heuristic for how far away an element is in markup. Tabbing may not do as well here as it appears it should. Absolute navigation is not harmed by this.}
	\TaskDef{Low visual indicator of focus}{Amazon is one of many websites where, in Google Chrome, focusing a page element will provide no visual feedback. With this being the case, click the first search result.}{This test is likely to harm tabbing, where seeing the currently focused element is crucial to estimating how far the user is from the target, and also whether they are already on the target. Mistakes and backtracking may be seen if the situation is confusing enough. ChibiPoint produces its own focus indicators, so should not be so affected by the page markup.}
	\TaskDef{Scrolled element}{Megatokyo hosts a tall webcomic, which fills some screens. There are buttons beneath this, but it is necessary to first scroll to see these. Click one of these off-screen elements, `Archive'.}{This is mainly a confirmation of effectiveness; early iterations of ChibiPoint did not follow the page as it scrolled, so it was seen that this was a class of navigation that a system can have varying success at. The only reason this differs from `Visually late element' is because the position ChibiPoint is launched from affects the pointing journey; an expert user could scroll an element into the middle of the screen so that the crosshairs would begin in the right place, saving keypresses.}
	\TaskDef{Link surrounded by links}{the Table of Contents for a Wikipedia article contains many hyperlinks cramped together. Click one of these, `2.3 the modern computer'.}{This class of pointing could cause problems for ChibiPoint flyouts; many suggestions need to be squeezed into a small space. Though the keypress count might not be affected, reading time may be harmed. Additionally, in the case of tabbing, there will be more elements to tab past.}
\end{enumerate}

\label{sec:tasklimitations}
...
\label{chap:results}
\section{Introduction}
This chapter describes and analyses the results of our quantitative study. We determine how our web browser extension compares to tabbing navigation, and determine also the contribution of our novel algorithm for suggesting `clickables' (defined in \Cref{sec:clickablesExplained}).
\section{Overview}
...
Qualitative data was captured also: we summarize our observations of task performance. User feedback, however, is not recruited in full until the discussion that follows this analysis in \Cref{chap:discussion}.
...
The navigation tasks used do not capture completely isolated cases. However we feel that it is inherently difficult to represent the real web with isolated navigation cases. We explain the problem and our justifications in detail in \Cref{sec:tasklimitations}.
...
\begin{figure}[ht]
\centering\rotatebox{-90}{\includegraphics[height=\textwidth,width=\safefigheight,keepaspectratio]{figures/Exertion2.pdf}}
...
\begin{figure}[ht]
\centering\rotatebox{-90}{\includegraphics[height=\textwidth,width=\safefigheight,keepaspectratio]{figures/TimeGraph2.pdf}}
...
\label{sec:userpref}
User preferences are tabulated in \Cref{fig:quantpost}. 10 participants (83\%) chose CX+F as their preferred system. The only deviations from these were 2 participants who chose CX. No participant preferred Tabbing.
...
\section{Introduction}
The purpose of this chapter is to take the analyses and results of the previous chapter, and build with them a case for whether our hypotheses and goals from \Cref{sec:goals} are met. We support our cases with user feedback from the questionnaires.
...
The primary purpose of this evaluation was to ascertain whether our hypotheses, which pertain to system efficiency, were supported. We remind the reader again of those hypotheses (whose detail can be read in full at \Cref{sec:detailedhyp}):
...
\section{Conclusions}
With ChibiPoint, we contribute a pointing system that excels in many areas compared to tabbing navigation (the current standards-blessed method for mouseless web navigation). We meet all the goals described in \Cref{sec:goals}, itemized as follows:
...
\newcommand{\permissionslip}[1]{\centerline{\includegraphics[width=\textwidth]{permission/#1.jpeg}}}
\permissionslip{Zack}
...
\caption{\Cref{fig:prequestionnaire} (Continued)}
\end{figure}
\begin{figure}[ht]
\ContinuedFloat
...
\caption{\Cref{fig:postquestionnaire} (Continued)}
\end{figure}

\FloatBarrier
\subsection{Instructions}
\label{sec:instructions}
\newcommand{\somesheet}[1]{\centering\includegraphics[width=\textwidth,height=\safefigheight,keepaspectratio]{#1}}
...
\begin{figure}[ht]
\instructionssheet{ChibiPointFlyoutInstructions}
\caption{Instructions for the `flyouts' feature of ChibiPoint.}
\label{fig:instructions_flyouts}
\end{figure}

\begin{figure}[ht]
\instructionssheet{TabbingInstructions}
\caption{Instructions for `tabbing navigation'.}
\label{fig:instructions_tabbing}
\end{figure}
\FloatBarrier

\subsection{Scripts}
\begin{figure}[ht]
\somesheet{sheets/script.pdf}
\caption{Script for researcher to follow.}
\label{fig:myscript}
\end{figure}

\begin{figure}[ht]
\somesheet{sheets/briefingScript.pdf}
\caption{Briefing script for participant.}
\label{fig:theirscript}
\end{figure}

\begin{figure}[ht]
\somesheet{sheets/taskScript.pdf}
\caption{Task script to prompt participant.}
\label{fig:taskscript}
\end{figure}
\FloatBarrier

\subsection{Test websites}
...
%(textwidth-3.5cm)/8
\newcommand{\psize}{0.125\textwidth-0.4375cm}
...
\newcommand{\mycodeincludejsb}[2]{
\mycodeinclude{language=custom, alsolanguage=JavaScript, style=whatever, #2}{#1}
}
...
% \begin{multicols}{2}
% \section{./#1}
% \lstinputlisting[language=custom, alsolanguage=JavaScript, style=whatever]{code2/#1}
% \end{multicols}
% \end{landscape}
% }
...
% \mycodeincludejs{src/background.js}
% \mycodeincludejs{src/depend.js}
% \mycodeincludejs{src/setup.js}
% \mycodeincludejs{src/lookup.js}

% \section{[Source Directory] Classes}
% \mycodeincludejs{src/Grid.js}
% \mycodeincludejs{src/Crosshairs.js}
% \mycodeincludejs{src/Flyout.js}

% \section{[Source Directory] Main Content}
% \mycodeincludejs{src/test.js}

% \section{[Source Directory] Evaluation-Only}
% \mycodeincludejs{src/evaluator.js}

% \section{[Source Directory] Debug}
% \mycodeincludejs{src/testonly.js}
% \mycodeincludejs{src/trace.js}

% \section{[Source Directory] No Longer Used}
% \mycodeincludejs{src/main.js}
% \mycodeincludejs{src/flyoutworker.js}

% \section{[External Libraries] DOM Selection}
% \mycodeincludejs{src/lib/jquery-2.1.0.min.js}

% \section{[External Libraries] Dependency Management}
% \mycodeincludejs{src/lib/require-cs.js}
% \mycodeincludejs{src/lib/require.js}

% \section{[External Libraries] Evaluation-Only}
% \mycodeincludejs{src/lib/FileSaver.js}

% \section{[External Libraries] No Longer Used}
% \mycodeincludejs{src/lib/Blob.js}
% \mycodeincludejs{src/lib/within.js}
...
\mycodeincludejsb{src/depend.js}{firstnumber=1, linerange={1-48}, captionpos=t, caption={Code for determining which elements have DOM Level 2 event listeners bound to them.}, label={lst:handlerTypes}}
\FloatBarrier
...
\mycodeincludejsb{src/test.js}{firstnumber=476, linerange={476-516}, captionpos=t, caption={Code for producing shortlist of clickables.}, label={lst:codeshortlist}}
...
\mycodeincludejsb{src/test.js}{firstnumber=387, linerange={387-411}, captionpos=t, caption={Code for querying the webpage for clickables.}, label={lst:detectclickables}}
