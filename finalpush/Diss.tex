...
\abstract
The web is designed for pointing devices like mice. Many users cannot use such input devices, due to disability or device restrictions. The existing accessible standard for mouseless pointing on the web is `tabbing navigation', which we find to be ineffective, unpredictable and inefficient. In order to solve this problem, we design, develop and evaluate `ChibiPoint': a pointing mechanism designed for modern websites. ChibiPoint is observed to be more effective and predictable than tabbing. Moreover, we demonstrate statistically that ChibiPoint is more efficient in a majority of navigation contexts. The architecture used for developing this browser extension allows its use to be extended to a wide variety of web browsers, devices and input hardware.
\newpage
...
An example of our system generating a shortlist of clickables with this algorithm can be seen in \Cref{fig:origClickable}.
