...
//chrome.tabs.onUpdated.addListener(f£unction(tabId, changeInfo, tab) {
  // No tabs or host permissions needed!
  //console.log('Turning ' + tab.url£+ ' red!');
  //chrome.tabs.executeScript({
    //code: 'document.body.style.back£groundColor="red"'
  //});
//});
...
var styles = '\
  ...
  var head = document.getElementsByTa£gName('head')[0];
  if (head) {
    var style =£document.createElement("style");
    style.type = "text/css";
    style.appendChild(document.create£TextNode(css));
...
//setAlternativeActiveDocument(docume£nt);
//options = default_options;

if (typeof(chrome) == "object" &&£chrome.extension) {
  //chrome.extension.sendRequest({'ge£t_options': true}, function(response)£{
    main();
  //});
} else {  
  main();
...
function is_shortcut(ev) {
    ...
    var is_windows = navigator.appVer£sion.indexOf("Windows") !== -1;
...
function init() {
  ...
  function getLatestSelector() {
    return selectHistory[selectHistor£y.length-1];
  }
	...
	function setEvents(rootNode) {
		...
		if (!body || !body.addEventListener) {
			return;
                ...
                setAlternativeActiveD£ocument(document);
		
		// usually on keydown
		//processSearch();
		//draw();
      var container = makeGridContain£er(document.documentElement);
...
trace($(getLatestSelector()+"£.grid").get(0));
            ...
            selectArray = [];
            selectArray.push(getLates£tSelector());
...
selectHistory.push(selectArray.join("£"));
            ...
            createGrid($(getLatestSel£ector()).first());
...
$(getLatestSelector()).empty();
            ...
            $(getLatestSelector()+"£.grid .row").each(function(index) {
                var i = index;
                $(this).children().ea£ch(function(index) {
                  var p = index;
                  if (cellHeight>12) {
$(this).text(7-(i*3-p));
...
$(this).append(trans);
        ...
        if (code == keycodes.dot) {
          ...
          if ($(".gridContainer").chi£ldren().length == 0) {
            // need to make a grid
            selectHistory =£[".gridContainer"];
            createGrid(container);
          } else {
...
function draw() {
  ...
  if (!box) { 
    ...
    document.documentElement.appendCh£ild(box);
    addStyle(styles);
  }
