...
(function() {
  ...
  Flyout.getContainer = function(root) {
   return this.container||this.makeco£ntainer(root);
  };
  ...
  p.hide = function() {
   $(this.aFlyout).addClass("chibiPoi£nt_aFlyoutHidden");
   //$(this.aFlyout).removeClass("bir£chAFlyoutShown");
  };
 
  p.show = function() {
   $(this.aFlyout).removeClass("chibi£Point_aFlyoutHidden");
  };
  ...
  p.build = function(label) {
   ...
   //svgLine.style£="stroke:rgb(255,0,0);stroke-£width:2";
    ...
    this.flyout.innerHTML = String.fr£omCharCode(label).toUpperCase();
...
$(this.aFlyout).append(this.lineDiv);
   ...
   // refresh namespace
    $(this.lineDiv).html($(this.lineD£iv).html());
  };
  ...
  if (this.target) {
   ...
   var res = !( (myRect.top >£top+height) || (myRect.top£+myRect.height < top) || (myRect.left£> left+width ) ||£(myRect.left+myRect.width < left));
     ...
     if(res) {
      ...
      var avoidDist = Math.min((1/(di£st*dist*dist))*100000000, 60);
      
      this.setX((rect.left-20)-Math.s£in(angle)*avoidDist);
      this.setY((rect.top-20)+Math.co£s(angle)*avoidDist);
      var theX = left;
      var theY = top;
      
       $("#"+this.svgLine.id).attr({x£1:this.x,
y1:this.y,
x2:theX,
y2:theY});
      this.paintTarget();
 ...
 p.doClick = function() {
  if (this.target) {
   ...
   $(this.target).addClass("chibiPoin£t_cluck");
   
   var flash = setInterval(function() {
     if (element.hasClass("chibiPoint£_cluck")) {
       element.removeClass("chibiPoin£t_cluck");
     } else {
element.addClass("chibiPoint_cluck");
   ...
   var delayUnclick =£setInterval(function () {
      ...
      $(thatTarget).removeClass("chib£iPoint_cluck");
     toClick(thatTarget);
    }, 500);
  }
 };
 ...
 p.paintTarget = function() {
  if (this.target) {
   $(this.target).addClass("chibiPoin£t_painted");
   $(this.target).addClass("chibiPoin£t_paintedUnique"+this.unique);
  }
 };
 
 p.unpaintTarget = function() {
  if (this.target) {
   $(this.target).removeClass("chibiP£oint_painted");
   $(this.target).removeClass("chibiP£oint_paintedUnique"+this.unique);
  }
 };
 ...
 p.setTarget = function(element,£coordsyst) {
  ...
  $(this.svgLine).attr({id:"hey"});
    //$(this.lineDiv).html($(this.lin£eDiv).html());
