define(["lib/jquery-2.1.0.min",£"lib/within", "lib/Blob",£"evaluator", "trace", "lookup",£"testonly", "Grid", "Crosshairs",£"Flyout"], function (jq, within,£blob, evaler, tracer, lookup,£testonly, gridclass, crosshairclass,£flyout) {
  ...
  function makeContainers() {
    ...
    if (birchlabs.flyoutsOn) {
      Flyout.makecontainer(document.d£ocumentElement);
      var flyouts = [];
    }
    Flyout.hide();
...
"1":[122,1], "2":[120,2],£"3":[99,3]};
    ...
    //createGrid();
      function setEvents(rootNode) {
          ...
          lookup.setAlternativeActive£Document(document);
        ...
        liveTargeter = function() {
          if (resize_ok === true) {
                resize_ok = false;
//trace($window.width());
              //trace($(".verticalHai£r").first().offset().left);
              //highlightTarget();
...
evaler.evaluatorIncrementKeys(code);
            if£(birchlabs.evaluateTabMode) {
              evaler.evaluatorWriteSt£ate(document.activeElement);
            } else {
              gridclick(grid);
...
evaler.evaluatorIncrementKeys(code);
                  evaler.evaluatorWri£teState(flyouts[i].getTarget());
                  flyouts[i].doClick();
      ...
      var rootNodes = [document].conc£at(lookup.getRootNodes());
...
setEvents(rootNode.contentDocument ?£rootNode.contentDocument : rootNode);
  ...
  function getDocRoot() {
    var rootNodes = [window].concat(l£ookup.getRootNodes());
...
if(careElements[this.nodeName]) found£= true;
        ...
        if (found) {
          //if ($(this).css("visibili£ty").indexOf("hidden") != -1 || !$(th£is).css("display").indexOf("none") !=£-1) return;
...
//"command":true,
      ...
      var rect = grid.getLastGrid().g£et(0).getBoundingClientRect();
      
      var rects = [];
      for (var i=0; i<3; i++) {
        for (var j=0; j<3; j++) {
          rects.push(grid.getLastRows£().eq(i).children().eq(j).get(0).getB£oundingClientRect());
        }
      }
      if (birchlabs.flyoutsOn) {
        var buckets =getAllClickables£InBoundsAndSort($(doc2.body), rect,£rects);
      ...
      for (var i=0; i<3; i++) {
        for (var j=0; j<3; j++) {
          ...
          for (var b=0;£b<buckets.length; b++) {
            ...
            var myBucket = buckets[(b£ucketIndex+b)%buckets.length];
...
f.point(grid.findPointerCoords());
  ...
  function createGrid(root) {
    ...
    // which on a low-density monitor£is 12pt
      ...
      var rect = $(root).get(0).getBo£undingClientRect();
      ...
      for (var i = 0; i < 3; i++) {
          ...
          for (var p = 0; p < 3; p++) {
              ...
              if (cellHeight>12) {
                cell.innerHTML = Stri£ng.fromCharCode(numpadMappings[(7-(i*£3-p)).toString()][0]).toUpperCase();
  ...
  function drill(num, grid,£crosshairs) {
    ...
    grid.getLastGrid().addClass("chib£iPoint_gridBorderOff");
    grid.getLastCells().addClass("chi£biPoint_cellBorderOff");
    grid.getLastGrid().removeClass("c£hibiPoint_gridBorderOn");
    ...
    selectArray = [];
    selectArray.push(grid.getLatestSe£lector());
...
grid.pushHistory(selectArray.join("£"));
    ...
    createGrid($(grid.getLatestSelect£or()).first());
...
$(grid.getLatestSelector()).empty();
      ...
      var cellHeight =£anticipatedHeight/3;
      grid.getLastRows().each(functio£n(index) {
          var i = index;
          $(this).children().each(fun£ction(index) {
            var p = index;
            if (cellHeight>12) {
              $(this).text(String.fro£mCharCode(numpadMappings[(7-(i*3-p)).£toString()][0]).toUpperCase());
      ...
      grid.getLastGrid().addClass("ch£ibiPoint_gridBorderOn");
      
      // if it's not too small, give£it borders
      var rect = $(grid.getLatestSele£ctor()).get(0).getBoundingClientRect(£);
      if (rect.width>60 &&£rect.height >60) {
  ...
  function gridclick(grid) {
    ...
    for (var i = 0; i <£rootNodes.length; i++) {
      ...
      element.removeClass("chibiPoint£_targeted");
element.addClass("chibiPoint_cluck");
      
      var flash =£setInterval(function() {
        if (element.hasClass("chibiPo£int_cluck")) {
          element.removeClass("chibiP£oint_cluck");
        } else {
element.addClass("chibiPoint_cluck");
        }
      }, 100);
...
//document.documentElement.focus();
      ...
      if (gridIsEmpty()) {
        // need to make a grid
        grid.initialize();
        $(".chibiPoint_gridContainer"£).removeClass("chibiPoint_hiddenGridC£ontainer");
  ...
  function closeGrid() {
    ...
    $(".chibiPoint_gridContainer").ad£dClass("chibiPoint_hiddenGridContaine£r");
  ...
  function gridIsEmpty() {
    return $(".chibiPoint_gridContain£er").children().length == 0;
  }
  
  function gridIsInUse() {
    //var grid = birchlabs.grid;
    return $(".chibiPoint_grid").chil£dren().length > 0;
  }
